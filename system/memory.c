...
static const char *qemu_sprint_backtrace(char *buffer, size_t length)
{
    char *p = buffer;
    if (current_cpu) {
                                    ...
                                    unsigned size)
{
    if (trace_unassigned) {
        char buffer[256];
        printf("Unassigned mem read " HWADDR_FMT_plx " %s\n",
                                 ...
                                 uint64_t val, unsigned size)
{
    if (trace_unassigned) {
        char buffer[256];
        printf("Unassigned mem write " HWADDR_FMT_plx
               " = 0x%" PRIx64 " %s\n",
